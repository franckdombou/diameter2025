...
public class ExampleClient implements EventListener<Request, Answer> {
    ...
    private static void configLog4j() {
        InputStream inStreamLog4j = ExampleClient.class.getResourceAsStream("log4j.properties");
    ...
    /**
    *Cette méthode initialise le stack Diameter en utilisant les fichiers de configuration spécifiés.
    * Elle charge également le dictionnaire AVP,
    * configure le stack, et ajoute un EventListener pour écouter les réponses du serveur
    */
    private void initStack() {
        ...
        try {
            ...
            log.info("Stack Configuration successfully loadedaw.");
    ...
    /**
     *Cette méthode démarre le client en créant une nouvelle session et en envoyant la première requête au serveur.
     */
    private void start() {
        try {
    ...
    /**
     * Cette méthode crée une nouvelle requête, ajoute les AVPs nécessaires,
     * puis envoie la requête au serveur. Elle est appelée à chaque étape de l'échange de messages avec le serveur.
     * */
    private void sendNextRequest(int enumType) throws InternalException, IllegalDiameterStateException, RouteException, OverloadException {
       ...
       // Avp exchengeData = requestAvps.addAvp(exchangeDataCode, TO_SEND[toSendIndex++], vendorID, true, false, false); // value
        // Ajouter les données attendues dans la demande
        Avp exchengeData = requestAvps.addAvp(exchangeDataCode, "Data you want to send DOMBOU", vendorID, true, false, false);
    ...
    /**
     *Cette méthode est utilisée pour afficher des informations détaillées sur les messages échangés entre le client et le serveur.
     * */
