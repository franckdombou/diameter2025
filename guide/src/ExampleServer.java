...
public class ExampleServer implements NetworkReqListener {
    ...
    private static void configLog4j() {
        InputStream inStreamLog4j = ExampleServer.class.getResourceAsStream("log4j.properties");
    ...
    private void initStack() {
        ...
        InputStream is = null; //Un InputStream est utilisé pour lire les données binaires à partir d'une source.
        try {
    ...
    public Answer processRequest(Request request) {
        ...
        if (exchangeDataAvp != null) {
            try {
                exchangeData = exchangeDataAvp.getUTF8String();
            } catch (AvpDataException e) {
                log.error("Error while extracting exchange data: " + e.getMessage());
            }
        }

        // Afficher les données envoyées par le client
        log.info("Data sent by client: " + exchangeData);
